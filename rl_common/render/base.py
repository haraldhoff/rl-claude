"""Shared pygame rendering machinery (backend-agnostic).
...
event handling.  It only ever sees host numpy arrays: every environment, on
either backend, exposes ``render_state()`` returning a dict that always carries
``steps`` and ``ep_return`` plus whatever that environment's renderer needs.

An environment's renderer subclasses this and implements :meth:`draw_tile`.
"""
...
class TiledRenderer:
    """Renders one or more environments of a vectorized env side by side."""
    ...
    def render(self) -> np.ndarray | None:
        ...
        self.state = self.env.render_state()
        self.steps = self.state["steps"]
        self.returns = self.state["ep_return"]
